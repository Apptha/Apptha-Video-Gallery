      ...
      if (! empty ( $file )) {
        ...
        if (strpos ( $file, '/' )) {
          ...
          if ($s3->putObjectFile ( $this->_srt_path . $file, $s3bucket_name, $file, S3::ACL_PUBLIC_READ )) {
            ...
            $result = 'https://' . $s3bucket_name . '.s3.amazonaws.com/' . $file;
          }
        }
      }
      ...
      if ($this->_addnewVideo) {
        ...
        if ($videoLinkurl != '') {
          ...
          if (preg_match ( '#https?://#', $videoLinkurl ) === 0) {
            $videoLinkurl = 'https://' . $videoLinkurl;
          }
        ...
        if ($videoLinkurl != '' && $act_filepath != '') {
          ...
          if ( (strpos ( $act_filepath, 'youtube' ) > 0 ) || (strpos ( $act_filepath, 'youtu.be' ) > 0) ) {
            ...
            $act_opimage  = $previewurl   = 'https://img.youtube.com/vi/' . $youtubeVideoID . '/maxresdefault.jpg';
            /** Get preview URL for YouTube videos */
            $act_image    = $img          = 'https://img.youtube.com/vi/' . $youtubeVideoID . '/mqdefault.jpg';
            ...
            $img          = $act_image = $act_opimage = $previewurl = 'https://www.dailymotion.com/thumbnail/video/' . $split_id [0];
          } else if (strpos ( $act_filepath, 'viddler' ) > 0) {
            /** Check video url is viddler
             * If yes, then get id from the url and get thumb, preview image */
            $imgstr     = explode ( '/', $act_filepath );
            $img        = $act_image = $act_opimage = $previewurl = 'https://cdn-thumbs.viddler.com/thumbnail_2_' . $imgstr [4] . '_v1.jpg';
          } else {
            $img = '';
          }
        } else {
        ...
        if (! empty ( $act_filepath )) {
          ...
          if (strpos ( $act_filepath, 'youtube' ) > 0 || strpos ( $act_filepath, 'youtu.be' ) > 0) {
            ...
            $act_image = 'https://i3.ytimg.com/vi/' . $match . '/mqdefault.jpg';
            /** Get previewimage for youtube video */
            $act_opimage = 'https://i3.ytimg.com/vi/' . $match . '/maxresdefault.jpg';
            ...
            $act_image = $act_opimage = 'https://www.dailymotion.com/thumbnail/video/' . $split_id [0];
          } else if (strpos ( $act_filepath, 'viddler' ) > 0) {
            /** Get viddler video id */
            $imgstr = explode ( '/', $act_filepath );
            /** Get viddler video thumb and preview image */
            $act_image = $act_opimage = 'https://cdn-thumbs.viddler.com/thumbnail_2_' . $imgstr [4] . '_v1.jpg';
          } else {
            $imgstr = '';
          }
        } else {
