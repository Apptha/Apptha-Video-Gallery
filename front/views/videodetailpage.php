    ...
    class ContusVideoDetailView extends ContusReportVideoView {
        ...
        function hdflv_sharerender($arguments = array()) {
              ...
              /** Check mobile device is detected */
               ...
               if ($file_type == 3 || $file_type == 1) {
                    ...
                    echo $output;
                        ...
                        $output='<script src="http://api.dmcdn.net/all.js"></script><div 
                        ></div><script>var player = DM.player(document.getElementById("player"),{video: "'.$split_id[0].'",width: "100%",params: {html: 0,wmode: "opaque"},events: {playing: function(){onPlayerStateChange();}}});var done = false;function onPlayerStateChange(){if (!done) {currentVideoP('.$videoId.');done = true;}}</script>';
                        echo $output;
                    } else {
                ...
                if (strpos ( $videourl, 'youtube' ) > 0) {  echo unserialize ( $this->_settingsData->player_colors );
                $videoid = getYoutubeVideoID ( $videourl );
                    $output .='<div id="player"><iframe width="'.$width.'" height="'.$height.'" src="https://www.youtube.com/embed/'.$videoid.'?modestbranding=1&autoplay='.$configXML->autoplay.'&rel=0&showinfo=0" frameborder="0" allowfullscreen></iframe></div><script>currentVideoP('.$videoId.');</script>';
                        /** Generate youtube embed code for html5 player */
                    } else{   
                $output .= '<div id="flashplayer"><embed  src="' . $this->_swfPath . '" flashvars="' . $flashvars . '" allowfullscreen="true" allowscriptaccess="always" type="application/x-shockwave-flash" wmode="transparent" width="'.$width.'" height="'.$height.'"></div>';
            ...
            if ($this->_post_type === APPTHAVIDEOGALLERY || $this->_page_post_type === APPTHAVIDEOGALLERY) {
                ...
                /** Display embed code */
                if (strpos ( $videourl, 'youtube' ) > 0) {
                  $embed_code = '<embed width="'.$width.'" height="'.$height.'"
src="https://www.youtube.com/embed/'.$videoid.'?rel=0&modestbranding=1">';
                ...
                if ($fetched [0]->file_type == 5 && $fetched [0]->embedcode) {
                  $iframe_code = stripslashes ( $fetched [0]->embedcode );
                	...
                	/** Load iframe code */
                    if (strpos ( $videourl, 'youtube' ) > 0) {
                  $iframe_code = '<iframe width="'.$width.'" height="'.$height.'" src="https://www.youtube.com/embed/'.$videoid.'" frameborder="0" allowfullscreen></iframe>';
                }else{
