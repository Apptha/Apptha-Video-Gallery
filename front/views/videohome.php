          ...
          if ($file_type == 5 && ! empty ( $homeplayerData->embedcode )) {
            ...
            if ($mobile) {
              ...
              if (strpos ( $videoUrl, 'youtube' ) > 0) {
                    /** Iframe code for youtube videos */
                    $videoId1   = getYoutubeVideoID ( $videoUrl );
                    $div .= '<div id="player"><iframe width="'.$settingsData->width.'" height="'.$settingsData->height.'" src="https://www.youtube.com/embed/'.$videoId1.'?modestbranding=1&autoplay=0&rel=0" frameborder="0" allowfullscreen></iframe></div><script>currentVideoP('.$videoId.');</script>';
                    }else{ 
